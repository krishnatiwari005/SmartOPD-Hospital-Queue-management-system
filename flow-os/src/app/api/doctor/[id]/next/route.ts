...
import { callNext, getSystemState } from "@/lib/services";
...
export async function POST(
  ...
  try {
    ...
    const result = await callNext(id);

    if (!result || !result.success) {
      return NextResponse.json({ error: result?.error || "Error calling next" }, { status: 400 });
    }

    // Return the updated queue for the doctor UI using admin client
